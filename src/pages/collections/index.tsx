...
// Mock data for jewelry items (same as dashboard)
const mockJewelry = [
  {
    id: 1,
    code: 'GR001',
...
const AllJewelryPage = () => {
  ...
  const popularSearches = ['Gold Necklace', 'Diamond Earrings', 'Bridal Set', 'Traditional Ring'];

  useEffect(() => {
    if (searchTerm.length > 0) {
      const suggestions = mockJewelry
        .filter(item => 
  ...
  const filteredJewelry = useMemo(() => {
    let filtered = mockJewelry.filter(item => {
      ...
      return matchesSearch && matchesCategory && matchesMetalType && matchesOccasion && matchesType && matchesQuickFilter;
    });
    ...
    return filtered;
  }, [searchTerm, selectedCategories, selectedMetalTypes, selectedOccasions, selectedTypes, quickFilter, sortBy]);
          ...
          <div className="container mx-auto px-4 py-8">
            <div className="text-center">
              ...
              <p className="text-charcoal/70 max-w-2xl mx-auto">
                Discover our complete range of exquisite traditional Indian jewelry, 
                crafted with precision and passion for over seven decades.
              </p>
            </div>
          </div>
        </div>
